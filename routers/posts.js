...
router.get('/posts', async (ctx, next) => {
  // decodeURIComponent对encodeURIComponent进行解码
...
router.post('/create', async (ctx, next) => {
  ...
  await userModel.insertPost([name, newTitle, newContent, md.render(newContent), id, time, avator])
    .then(() => {
      ctx.body = true
    }).catch(() => {
      ctx.body = false
...
router.post('/posts/:postId/edit', async(ctx, next) => {
  ...
  await userModel.updatePost([newTitle, newContent, md.render(newContent), postId])
      .then(() => {
          ctx.body = true
      }).catch(() => {
          ctx.body = false
